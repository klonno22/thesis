...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{graphics/sign-in-oauth2.pdf}
...
Nach Bearbeitung der Thesis gibt es in Blattwerkzeug drei unterschiedliche Bereiche die ein Benutzer besuchen kann. Das Adminpanel welches ohne jegliche Berechtigung besucht werden konnte, den öffentlichen Bereich und die Einstellungen eines Benutzers.

Damit bei der Darstellung von Inhalt unterschieden werden kann, wurden drei globale Rollen implementiert. 

Für die Unterscheidung der Berechtigung eines Benutzers wurden drei globale Rollen implementiert. 

\todo[inline]{Unklar: Was genau wurde extra \enquote{eingebaut}? Ich halte das eher für eine Konvention. Und zwischen Benitzern unterscheidet man anhand ihrer ID ;-) Ich wei{\ss} was gemeint ist, aber geh hier nochmal in dich und formulier sauber aus inwiefern diese (sinnvolle!) Konvention dir hilft.}
